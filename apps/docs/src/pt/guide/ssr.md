...
Com Laravel + Inertia, o mesmo vale para o `resources/js/ssr.ts`. Veja [Laravel + Inertia](/pt/guide/laravel-inertia).
...
O `useSystemColorMode()` só lê `matchMedia` depois da montagem. Até lá, `resolvedColorMode` vale `light` no servidor e no cliente. Para respeitar uma escolha explícita (`light` ou `dark`) sem flash, veja [Modo escuro](/pt/guide/dark-mode#sem-flash-no-ssr).
...
- [Modo escuro](/pt/guide/dark-mode)
- [Idiomas (i18n)](/pt/guide/i18n)
- [Toast](/pt/components/toast)
