...
Todos os temas compartilham o mesmo visual padrão, no estilo shadcn/ui: controles de 36px, sombras discretas e anel de foco com halo. Cada tema troca cores, raios, profundidade e fontes. Compare os temas na [galeria](/pt/themes) e todos os tokens na [referência de tokens](/pt/tokens).
...
Os dials funcionam em qualquer elemento, não só junto de `data-ms-theme`. Para mudar tudo isso em tempo de execução, use `useThemeContext()` (veja [Modo escuro](/pt/guide/dark-mode)).
